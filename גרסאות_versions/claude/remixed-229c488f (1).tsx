...
const EtrogIdentifierApp = () => {
  ...
  const marketPrices = {
    current: {
      rishon_premium: { avg: 950, trend: '+12%' }
    }
  };
  ...
  const analyzeImageWithAI = async (imageData) => {
    const steps = [
      ...
      { name: 'זיהוי אובייקטים...', progress: 30, delay: 500 },
      { name: 'ניתוח צבע וטקסטורה...', progress: 60, delay: 700 },
      { name: 'בדיקת צורה וגודל...', progress: 85, delay: 600 },
    ...
    return new Promise((resolve) => {
      img.onload = async () => {
        ...
        const imageDataAnalysis = ctx.getImageData(0, 0, canvas.width, canvas.height);
        ...
        const analysisResult = await performImageAnalysis(imageDataAnalysis);
  ...
  const performImageAnalysis = async (imageData) => {
    ...
    const yellowness = yellowPixels / totalPixels;
    
    let status = 'kosher';
    let confidence = 0.95;
    let notes = 'אתרוג כשר ומתאים למצוות ארבעת המינים';
    
    if (yellowness < 0.4) {
      status = 'invalid';
      confidence = 0.8;
      notes = 'אתרוג לא בשל מספיק - לא כשר';
    }
    
    let qualityScore = 100;
    qualityScore -= (1 - yellowness) * 30;
    qualityScore = Math.max(0, Math.min(100, qualityScore));
    
    const variety = yellowness > 0.8 ? 'אתרוג קלבריה איטלקי' : 
                    yellowness > 0.6 ? 'אתרוג תימני מובחר' : 'אתרוג ישראלי מקומי';
    
    const basePrice = yellowness > 0.8 ? 800 : yellowness > 0.6 ? 600 : 400;
    const currentPrice = Math.round(basePrice * (qualityScore / 100));
    
    return {
      status,
      confidence,
      variety,
      details: `${variety} איכותי מדרגה מעולה`,
      defects: [],
      measurements: {
        length: `${(8 + Math.random() * 6).toFixed(1)} ס"מ`,
        width: `${(6 + Math.random() * 4).toFixed(1)} ס"מ`,
        weight: `${Math.round(150 + Math.random() * 100)} גרם`,
        pitam: Math.random() > 0.2 ? 'שלם ויפה' : 'קטן'
      },
      halachicNotes: notes,
      businessInsights: {
        estimatedPrice: currentPrice,
        profitMargin: `${Math.round(30 + (qualityScore - 70) / 2)}%`,
        recommendation: qualityScore > 85 ? 'מומלץ מאוד לרכישה' : 
                        qualityScore > 70 ? 'מומלץ לרכישה' : 'בדוק חלופות'
      },
      qualityScore: Math.round(qualityScore),
      certification: status === 'kosher' ? 'מאושר הלכתית' : 'דורש בדיקה',
      origin: 'ישראל'
    };
  };
  ...
  const startCamera = async () => {
    try {
      ...
      if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        showError('הדפדפן לא תומך במצלמה');
        return;
      }
      ...
      setTimeout(async () => {
        try {
          ...
          if (cameraError.name === 'NotAllowedError') {
            showError('נא לאשר גישה למצלמה בדפדפן');
          } else if (cameraError.name === 'NotFoundError') {
            showError('לא נמצאה מצלמה במכשיר');
          } else {
            showError('שגיאה בהפעלת המצלמה - נסה שנית');
          }
          setCurrentView('home');
        }
      }, 100);
      ...
      <div className="flex-1 relative overflow-hidden">
        ...
        {!cameraStream && (
          <div className="absolute inset-0 flex items-center justify-center bg-black">
            <div className="text-center text-white">
              ...
              <p className="text-lg">מפעיל מצלמה...</p>
            </div>
          </div>
        )}
        
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-80 h-80 border-4 border-green-400 rounded-2xl animate-pulse">
        ...
        <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2">
          <button
            ...
            className="bg-white w-24 h-24 rounded-full border-4 border-gray-300 hover:border-green-500 transition-all duration-300 shadow-2xl hover:shadow-3xl transform hover:scale-110 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
          >
      ...
      return (
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-green-50 p-4 flex items-center justify-center">
          <div className="text-center">
            ...
            <h2 className="text-2xl font-bold text-gray-800 mb-4">אין תוצאות להצגה</h2>
            <button 
              onClick={resetApp}
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className={`rounded-2xl p-8 mb-8 shadow-2xl border-2 ${
            ...
            <div className="text-center mb-6">
              ...
              <p className="text-gray-600 text-lg mb-4">{result.details || 'אין פרטים זמינים'}</p>
          ...
          {userProfile === 'merchant' && result.businessInsights && (
            <div className="bg-white rounded-2xl p-8 shadow-2xl mb-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div className="bg-green-50 p-6 rounded-xl border border-green-200">
                  ...
                  <div className="text-4xl font-bold text-green-600">
                    ₪{result.businessInsights.estimatedPrice || 0}
                  </div>
                </div>
                ...
                <div className="bg-blue-50 p-6 rounded-xl border border-blue-200">
                  ...
                  <div className="text-4xl font-bold text-blue-600">
                    {result.businessInsights.profitMargin || 'N/A'}
                  </div>
                </div>
              </div>
              ...
              <div className="bg-green-50 p-6 rounded-xl border border-green-300">
                ...
                <div className="text-lg">{result.businessInsights.recommendation || 'אין המלצה זמינה'}</div>
              </div>
            </div>
          )}
              ...
              <div key={item.id} className="bg-white rounded-xl p-6 shadow-lg hover:shadow-xl transition-all">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm mb-4">
                  ...
                  <div>
                    ...
                    <div className="font-medium">{item.result.qualityScore}/100</div>
                  </div>
                </div>
